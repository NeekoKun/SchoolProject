...
## Colors
axes_color = (255, 255, 255, int(0.5 * 255))
...
tracking_layer   = pygame.Surface(SIZE, pygame.SRCALPHA)
...
def display_tracking_points(in_tracking_points):
    ...
    for tracking_point in in_tracking_points:
        for i in range(len(tracking_point) - 1):
            ...
            point = (int(coors1[0]), int(coors1[1]))
            new_point = (int(coors2[0]), int(coors2[1]))
            pygame.draw.line(tracking_layer, (100, 200, 100), point, new_point, max(int(speed), 1))
...
def display_axes():
    ...
    for i in x_tick_coordinates: # Calculate x axis ticks
        ...
        if i != 0:
            text = tick_font.render(str(int(i)), True, axes_color)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1 and pause_button.collidepoint(event.pos):
                paused = not paused
            elif event.button == 1:
    ...
    screen.blit(tracking_layer, (0, 0))
    pygame.draw.rect(screen, (200, 200, 200), pause_button) if paused else pygame.draw.rect(screen, (100, 100, 100), pause_button)
